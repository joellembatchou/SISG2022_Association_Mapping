...
\begin{itemize}
	...
	$$ \mu_i/\log\left(\frac{\mu_i}{1-\mu_i}\right)=\alpha_0  + \bX_i' \balpha + \bf G_i' \bbeta    $$
...
%		{\large Wu et al., (2010, 2011).  {\it AJHG}}
	...
	\begin{itemize}\setlength{\itemsep}{1.2ex}
		...
		\begin{align}
			& \mu_i/\log\left(\frac{\mu_i}{1-\mu_i}\right)= \alpha_0  + \bX_i^T \balpha + {\bf G}_i^T \bbeta \notag 
		\end{align}
...
%		where $U_j = \sum_{i=1}^n g_{ij} (y_i - \muhat_{0i}) $.
		...
		{  $$\mu_i/\log\left(\frac{\mu_i}{1-\mu_i}\right)= \alpha_0  + \bX_i^T \balpha + g_{ij} \beta_j   $$}
...
\frame{
	...
	\vspace{0.4cm}
	\pause
	{\Red We want to develop a unified test that works well in both situations} $\rightarrow$ Omnibus tests
}
...
\frame{\frametitle{SKATO} 
	\begin{itemize}
		...
		\item SKATO p-value is obtained through numerical integration
	\end{itemize}
}

\frame{\frametitle{SKATO vs Burden/SKAT}
	
	\begin{itemize}
		\item SKATO remains powerful across all scenarios
		\begin{figure}
...
\frame{\frametitle{Aggregated Cauchy Association Tests: ACATV \& ACATO} 
	\begin{itemize}
		\item ACATV
				\begin{itemize}
	...
	\item ACATO
				\begin{itemize}
\item Apply ACAT to combine the p-values of SKAT, Burden and ACATV
...
\frame{\frametitle{ACAT/SKAT/Burden }
	...
	\begin{itemize}
		\item ACATO remains powerful across all scenarios
		\begin{figure}
...
\frame{\frametitle{Summary} 
	...
	\begin{itemize}
		\item Region based tests can increase the power of rare variants analysis.
		\item Relative performance of rare variant tests depends on underlying disease models 
		\item  Combined tests (omnibus tests), e.g, SKATO/ACATO, are more robust and powerful in different scenarios 
	\end{itemize}
