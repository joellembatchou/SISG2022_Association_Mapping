---
title: 'Practical 9: Within-family GWAS'
...
gwas_pcs <- do.call("rbind",lapply(1:M, function(j){
  summary(lm(Y1~X1[,j] + pcs[,1:10]))$coefficients[2,]
}))
...
**Question 2. Increase the number of subgroups (e.g.. `g=5`, `g=10`, `g=50`, `g=100`). What can you say about the separation between subgroups on the first two PCs? Is the adjustment for 10 PCs still sufficient to control for inflation? If not, how many PCs do you need to fit and why? What do you observe for the within-family GWAS results?**

**Question 3. Now set `Fst=0.1` (that would correspond to subgroups with different continental ancestries), `vs=0.1` and `g=100`. Are your conclusions from Question 2 different?**
