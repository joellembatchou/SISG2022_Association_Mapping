...
# Run PLINK1.9
system(paste0("/data/SISG2022M15/exe/plink --make-bed --simulate sim.config --simulate-ncases ", N, " --simulate-ncontrols 0 --simulate-prevalence 0.1  --out cc_imb_geno"))
```
...
```{r}
system("/data/SISG2022M15/exe/regenie --bed cc_imb_geno --phenoFile cc_imb_pheno.txt --step 2 --bsize 400 --bt  --ignore-pred --out regenie_step2")
```
...
*Extra: 5. Re-run GWAS in Questions 1 but now applying Firth correction. Make a QQ plot of the -log10 p-values for Y3.*
```{r}
system("/data/SISG2022M15/exe/regenie --bed cc_imb_geno --phenoFile cc_imb_pheno.txt --step 2 --bsize 400 --bt  --ignore-pred --firth --out regenie_step2_firth")
sumstats.y3.firth <- fread("regenie_step2_firth_Y3.regenie") %>% mutate(Pval = 10^(-LOG10P), Z = sign(BETA) * sqrt(CHISQ))
qqPlot( pval = sumstats.y3.firth$Pval )
```
